...
export default function ContentBlockComponent({ contentBlock, previewMode = false, onMoveBlock }: ContentBlockComponentProps) {
  ...
  const renderContent = () => {
    ...
    switch (contentBlock.type) {
      ...
      case "flashcards":
        return (
          <div className="grid md:grid-cols-2 gap-4" data-testid={`content-flashcards-${contentBlock.id}`}>
            {content.cards?.slice(0, 2).map((card: any, index: number) => (
              <button
                type="button"
                key={index}
      ...
      case "ai-audio":
        ...
        return (
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" data-testid={`content-ai-audio-${contentBlock.id}`}>
            <div className="flex items-start space-x-4 mb-3">
              <div className="text-2xl font-semibold text-slate-300">AI</div>
              <div className="flex-1">
                <h4 className="font-medium text-slate-900">{content.title || "AI Generated Audio"}</h4>
                <p className="mb-2 text-sm text-slate-600">{content.description || "Audio narration generated by AI"}</p>
                {content.duration && (
                  <p className="mb-2 text-xs text-slate-500">Duration: {content.duration}</p>
                )}
                {content.script && (
                  <div className="mt-3">
                    <p className="mb-1 text-xs font-medium text-slate-700">Script Preview:</p>
                    <p className="max-h-20 overflow-y-auto rounded-xl bg-slate-50 p-2 text-xs text-slate-600">
            ...
            {!hasValidAiAudioUrl && (
              <div className="mt-2 rounded-xl bg-slate-50 p-3">
                <p className="text-xs text-slate-600">
                  ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¹ÃƒÂ¯Ã‚Â¸Ã‚Â Audio generation is currently using placeholder URLs. To enable actual audio playback:
                </p>
                <ul className="text-xs text-purple-600 dark:text-purple-400 mt-2 ml-4 list-disc">
      ...
      case "ai-quiz":
        return (
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" data-testid={`content-ai-quiz-${contentBlock.id}`}>
            <div className="flex items-start space-x-3 mb-4">
              <div className="flex-1">
                {cleanQuizTitle(content.title || "") && (
                ...
                {Array.isArray(content.questions) && content.questions.length > 0 ? (
                  <div className="space-y-4">
                    {content.questions.map((question: any, index: number) => (
                      <div key={question.id || index} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                  ...
                  <div className="py-4 text-center text-slate-500">
                    <p className="text-sm">AI-generated quiz questions will appear here</p>
                    <p className="text-xs opacity-75">Use the editing interface to generate questions</p>
                  </div>
                )}
              </div>
            </div>
      ...
      case "quiz":
        return (
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" data-testid={`content-quiz-${contentBlock.id}`}>
            <div className="flex items-start space-x-3 mb-4">
              <div className="flex-1">
                <h3 className="mb-2 font-semibold text-slate-900">
                  {content.title || "Quiz"}
                ...
                {Array.isArray(content.questions) && content.questions.length > 0 ? (
                  ...
                  <div className="py-4 text-center text-slate-500">
                    <p className="text-sm">Quiz questions will appear here</p>
                    <p className="text-xs opacity-75">Add questions manually to create your quiz</p>
                  </div>
                )}
              </div>
            </div>
      ...
      case "assignment":
        return (
          <div className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" data-testid={`content-assignment-${contentBlock.id}`}>
            {content.title && (
              <div>
            ...
            {Array.isArray(content.objectives) && content.objectives.length > 0 && (
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <h4 className="mb-2 font-medium text-slate-900">Learning Objectives:</h4>
            ...
            {Array.isArray(content.tasks) && content.tasks.length > 0 && (
              <div className="space-y-3">
                <h4 className="font-medium text-slate-800 dark:text-slate-100">Tasks</h4>
                <div className="space-y-3">
                  {content.tasks.map((task: any, index: number) => (
                    <div key={task.id || index} className="border border-slate-200 dark:border-slate-700 rounded-lg p-4 bg-white dark:bg-slate-900">
                      <div className="flex items-start gap-3">
                        <div className="flex-shrink-0 h-6 w-6 rounded-full bg-emerald-100 dark:bg-emerald-900 flex items-center justify-center text-sm font-medium text-emerald-700 dark:text-emerald-200">
                          {index + 1}
                        </div>
            ...
            {Array.isArray(content.resources) && content.resources.length > 0 && (
              <div className="space-y-2">
                <h4 className="font-medium text-slate-800 dark:text-slate-100">Resources</h4>
                <ul className="space-y-1">
      ...
      case "list":
        return (
          <div className="space-y-3" data-testid={`content-list-${contentBlock.id}`}>
            {content.title && (
      ...
      case "process-flow":
        return (
          <div className="space-y-4" data-testid={`content-process-flow-${contentBlock.id}`}>
            {content.title && (
              <h4 className="mb-4 font-medium text-slate-900">{content.title}</h4>
            )}
            {Array.isArray(content.steps) && content.steps.length > 0 ? (
  ...
  return (
    <BlockErrorBoundary blockId={contentBlock.id} blockType={contentBlock.type}>
      ...
      {!previewMode && (
        <div className="absolute -left-4 top-8 flex flex-col gap-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
